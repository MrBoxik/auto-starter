...
def create_startup_shortcut(target_executable, args_list=None):
    ...
    try:
        if HAVE_PYWIN32:
            ...
            # Optionally set the icon if exists next to the executable
            icon_path = os.path.join(os.path.dirname(target_executable), ICON_FILENAME)
...
def create_startup_task(target_executable, args_list=None):
    """
    ...
    try:
        ...
        if p.returncode == 0:
            return True
        print('Failed to create startup task:', p.stdout.strip(), p.stderr.strip())
        return False
    except Exception as e:
        print('Failed to create startup task:', e)
        return False
...
def remove_startup_task():
    ...
    try:
        ...
        print('Failed to remove startup task:', e)
        return False
...
class StarterApp:
    def __init__(self, root):
        ...
        # Try to set a window icon if app_icon.ico exists next to the script/exe
        try:
            ...
            if os.path.exists(icon_path) and sys.platform.startswith('win'):
                # iconbitmap works on Windows for .ico
                try:
                ...
                # Try generic PhotoImage (PNG) in same folder if someone renamed; ignore failures
                png_icon = os.path.join(base_dir, 'app_icon.png')
                if os.path.exists(png_icon):
                    try:
                        img = PhotoImage(file=png_icon)
                        self.root.iconphoto(False, img)
                    except Exception:
                        pass
        except Exception:
    ...
    def toggle_startup(self):
        if self.startup_var.get():
            ...
            if any(is_self_path(it.get('path')) for it in self.items if isinstance(it, dict)):
                ...
                return
            ok_task = create_startup_task(target, args_list)
            if ok_task:
    ...
    def upgrade_startup_entry_if_needed(self):
        ...
        target, args_list = get_startup_launch_target_and_args()
        if create_startup_task(target, args_list):
            remove_startup_shortcut()
